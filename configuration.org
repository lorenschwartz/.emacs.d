...
#+BEGIN_SRC emacs-lisp
  (setq org-capture-templates
    ...
    ("m" "minutes" entry (file+headline "~/Dropbox/org-files/Main.org" "Minutes")
     "** MEETING %^{Title} %^T %^G\n %?\n")
    ("j" "journal" entry (file+olp+datetree "~/Dropbox/org-files/Personal.org" "Journal")
     "** %?")
    ("o" "observation" entry (file+headline "~/Dropbox/org-files/Personal.org" "Personal Observations")
     "** %?\n %U\n")
    ("g" "goals" entry (file+headline "~/Dropbox/org-files/Personal.org" "Goals")
     "** GOAL %?\n %^{AREA}p")
    ("c" "contact" entry (file+headline "~/Dropbox/org-files/Personal.org" "Contacts")
...
#+BEGIN_SRC emacs-lisp
  (setq org-todo-keywords
    '((sequence "TODO" "NEXT(n)" "DOING(!)" "WAITING(w@/!)" "HOLD(h@/!)" "DELEGATED(D)" "|" "DONE(d)" "CANCELLED(X)")
     (sequence "OPEN(o)" "|" "CLOSED(l)")
     (sequence "IDEA(i)" "|" "ABANDON")
     (sequence "GOAL(g)" "|" "DONE")
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun ls/org-skip-subtree-if-habit ()
    "Skips entry it it has the habit style"
    (let ((subtree-end (save-excursion (org-end-of-subtree t))))
	  ...
	  ("c2" "Open Agenda View"
	   ((todo "OPEN"
		  ((org-agenda-sorting '(priority-down))
		   (org-agenda-overriding-header "Open Agenda Items")))))
	  ("c3" "Ideas"
	   ((todo "IDEA"
		  ((org-agenda-sorting '(priority-down))
		   (org-agenda-overriding-header "Idea Incubator")))))
	  ("c4" "Kanban Board Agenda View"
	   ((agenda "")
	    (todo "GOAL"
		  ((org-agenda-sorting '(priority-down))
		   (org-agenda-skip-function '(org-agenda-skip-entry-if 'todo 'done))
	    ...
	    (todo "NEXT"
		  ((org-agenda-skip-function '(org-agenda-skip-entry-if 'tags-todo "Goals"))
		   (org-agenda-sorting '(priority-down))
		   (org-agenda-overriding-header "Next Best Actions")))
	    (todo "DOING"
		  ((org-agenda-skip-function '(org-agenda-skip-entry-if 'tag-any-match-p '"Goals"))
		   (org-agenda-sorting '(priority-down))
		   (org-agenda-overriding-header "Doing")))
	    (todo "OPEN|DELEGATED"
		  ((org-agenda-sorting '(tag-down priority-down))
		   (org-agenda-skip-function '(org-agenda-skip-entry-if 'todo 'done))
		   (org-agenda-overriding-header "For Follow Up")))
	    (todo "WAITING|HOLD"
		   ((org-agenda-sorting '(priority-down))
	    ...
	    (todo "IDEA"
		  ((org-agenda-sorting '(tag-down priority-down))
		   (org-agenda-skip-function '(org-agenda-skip-entry-if 'todo 'done))
		   (org-agenda-overriding-header "The Incubator")))
	    (alltodo ""
		     ((org-agenda-skip-function
		       '(or
			 (org-agenda-skip-entry-if 'scheduled)
			 (org-agenda-skip-entry-if 'deadline);;(org-agenda-skip-entry-if (ls/org-skip-subtree-if-habit))
			 (org-agenda-skip-entry-if 'todo '("GOAL" "IDEA" "NEW" "PLANNING" "EXECUTION-ON TIME" "EXECUTION-LATE" "OPEN" "DOING" "NEXT" "WAITING" "HOLD" "DELEGATED"))))
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Right Alt (option) can be used to enter symbols like em dashes '-', etc.
  (setq mac-right-option-modifier 'nil)

  ;; Use ESC as universal get me out of here command
  (define-key key-translation-map (kbd "ESC")(kbd "C-g"))

  ;; macOS type key bindings
  ...
  (defun sensible-defaults/bind-commenting-and-uncommenting ()
    "Comment or uncomment a region by hitting S-;."
...
#+BEGIN_SRC emacs-lisp
  (use-package neotree
    ...
    (setq neo-window-width 32
	  ...
	  new-banner-message nil
	  new-show-updir-line t
	  new-window-fixed-size nil
...
#+BEGIN_SRC emacs-lisp
  (use-package cider
    ...
    ;; (add-hook 'cider-repl-mode-hook #'company-m)
...
(add-to-list 'custom-theme-load-path "~/.emacs.d/themes/Emacs-Tron-Legacy-Theme")
#+END_SRC
* Pomidor
#+BEGIN_SRC emacs-lisp
(use-package pomidor
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; use a global eslint and enable for web-mode
